...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Shareable Daily Challenge attempts — public, no auth.
    if (request.method === 'GET' && url.pathname.startsWith('/api/share/')) {
      return handleShareGet(request, env, cors, url.pathname.slice('/api/share/'.length));
    }
...
async function handleChallengeGrade(
  ...
  // ── Persist (signed-in: attempt + streak), mint share (everyone)
  ...
  // Share records are self-contained (problem date + student work + grade)
  // so anon and signed-in both get one. The opaque 64-bit hex id never
  // resolves to anything identifiable.
  const share = await createShare(env.USAGE, {
    date: record.date,
    challengeNumber: challengeNumberFor(record.date),
    studentMmd: studentWorkText,
    grade,
 ...
 * problem + the sharer's grade + their submitted work as MMD (rendered
 * inline by the SPA via KaTeX).
 *
 * The share record is self-contained — no userId is involved on either
 * the write or read side, so anonymous attempts share the same way as
 * signed-in ones.
 */
